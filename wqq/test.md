...
Понял, сейчас подготовлю полные ответы на практические задания в едином, читаемом формате, как и для ситуационных.
Начну с первых — чтобы не было слишком много в одном сообщении, сделаю по 3-4 варианта и дам файл для скачивания.
...
**Задание:** Настройте статический IP-адрес на компьютере с ОС Windows и подключите его к локальной сети. Проверьте связность с соседними устройствами.

**Ответ:**

1. Откройте «Панель управления» → «Центр управления сетями и общим доступом» → «Изменение параметров адаптера».
2. Правой кнопкой мыши кликните на нужном сетевом адаптере и выберите «Свойства».
3. Выберите «Протокол Интернета версии 4 (TCP/IPv4)» → «Свойства».
4. Установите «Использовать следующий IP-адрес»:

   * IP-адрес: например, 192.168.1.100
   * Маска подсети: 255.255.255.0
   * Основной шлюз: 192.168.1.1 (адрес роутера)
   * Предпочитаемый DNS-сервер: 8.8.8.8 или адрес локального DNS
5. Сохраните настройки и закройте все окна.
6. Откройте командную строку (cmd) и выполните проверку пингом соседних устройств:

   ```
   ping 192.168.1.101
   ping 192.168.1.1
   ```
7. Если пинги успешны, настройка завершена.

---

### Вариант 2

**Задание:** Создайте два виртуальных сегмента с VLAN на коммутаторе. Настройте порты так, чтобы устройства на одном VLAN не могли обмениваться данными с устройствами на другом VLAN.

**Ответ:**

1. Подключитесь к коммутатору (например, Cisco) через консоль.
2. Создайте VLAN:
...
3. Назначьте порты в VLAN:
   ...
   exit
   ```
4. Убедитесь, что меж VLAN связь отсутствует (обычно по умолчанию). Если маршрутизация включена — настройте ACL для блокировки.
5. Сохраните конфигурацию:

   ```
   write memory
   ```
6. Проверьте, что устройства в разных VLAN не видят друг друга.

---

### Вариант 3

**Задание:** Настройте DHCP-сервер на сервере с Windows Server. Убедитесь, что клиентский компьютер получает IP-адрес, подсеть и шлюз по умолчанию автоматически.

**Ответ:**

1. Установите роль DHCP-сервера через «Диспетчер серверов».
2. Откройте «DHCP» MMC и создайте новый диапазон IP-адресов (Scope):

   * Например, диапазон 192.168.1.100 – 192.168.1.200
   * Маска подсети 255.255.255.0
   * Основной шлюз (Default Gateway) 192.168.1.1
3. Активируйте DHCP-сервер и запустите службу.
4. На клиентском ПК настройте получение IP автоматически (DHCP).
5. Проверьте в командной строке клиента:

   ```
   ipconfig /renew
   ipconfig /all
   ```
6. Убедитесь, что клиент получил адрес из указанного диапазона, правильную маску и шлюз.

---

### Вариант 4

**Задание:** Выполнить установку и настройку программного обеспечения системы резервного копирования.

**Ответ:**

1. Выберите ПО для резервного копирования (например, Acronis, Veeam, Windows Server Backup).
2. Установите выбранное ПО на сервер или рабочую станцию согласно инструкции производителя.
3. Запустите программу и создайте новый план резервного копирования:

   * Выберите объекты для бэкапа (файлы, папки, системы).
   * Настройте расписание (ежедневно, еженедельно).
   * Укажите место хранения резервных копий (локальный диск, сетевой ресурс, облако).
4. Выполните тестовое резервное копирование и убедитесь в успешном завершении.
5. Настройте уведомления об успешном/неудачном бэкапе.
6. Проведите периодические проверки восстановления данных из резервной копии.

Продолжаю с вариантами практических заданий.

---

### Вариант 5

**Задание:** Настройте статическую маршрутизацию между двумя сетями на маршрутизаторе. Проверьте связность между компьютерами из разных сетей.

**Ответ:**

1. Определите IP-сети, которые нужно соединить, например:

   * Сеть 1: 192.168.1.0/24
   * Сеть 2: 192.168.2.0/24
2. На маршрутизаторе настройте интерфейсы с IP-адресами из соответствующих сетей.
3. Добавьте статические маршруты для каждой сети:

   ```
   ip route 192.168.1.0 255.255.255.0 <интерфейс или IP следующего хопа для сети 1>
   ip route 192.168.2.0 255.255.255.0 <интерфейс или IP следующего хопа для сети 2>
   ```
4. На компьютерах обеих сетей настройте шлюз по умолчанию на IP маршрутизатора в их сети.
5. Проверьте связь с помощью ping с компьютера из сети 1 в сеть 2 и наоборот.
6. Если пинг проходит, маршрутизация настроена корректно.

---

### Вариант 6

**Задание:** (Не проходили) Настройте ACL на маршрутизаторе, чтобы разрешить/запретить доступ к определенным ресурсам в сети. Проверьте правила, блокирующие/разрешающие трафик.

**Ответ:**

1. Определите правила доступа — что нужно разрешить или запретить (например, запретить доступ к IP 10.0.0.5).
2. Создайте список доступа (ACL), например:

   ```
   access-list 100 deny ip any host 10.0.0.5
   access-list 100 permit ip any any
   ```
3. Примените ACL к интерфейсу маршрутизатора в нужном направлении (входящий/исходящий):
...
5. Проверьте работу правил, пытаясь получить доступ к запрещённым и разрешённым IP с помощью ping или telnet.
6. Для детальной проверки используйте команды `show access-lists` и логи маршрутизатора.

---

### Вариант 7

**Задание:** Разработать подробную процедуру восстановления операционной системы, включая этапы, критерии успешного завершения и регулярные проверки.
...
1. **Подготовка:**
   ...
   * Подготовьте загрузочный носитель с ОС (USB/DVD).
2. **Этапы восстановления:**

   * Попытаться загрузиться в безопасном режиме (Safe Mode).
   * Использовать точку восстановления системы (System Restore).
   * Если не помогло — выполнить восстановление системы из резервной копии.
   * В крайнем случае — переустановка ОС с форматированием или без.
3. **Критерии успешного завершения:**

   * ОС загружается без ошибок.
   * Все необходимые драйверы и приложения работают корректно.
   * Данные восстановлены и доступны.
4. **Регулярные проверки:**

   * Плановое тестирование резервных копий.
   * Проверка работоспособности системы после обновлений.
...
**Задание:** Используя Wireshark, проанализируйте сетевой трафик в локальной сети. Определите типы пакетов, их источники и назначения.

**Ответ:**

1. Запустите Wireshark с правами администратора.
2. Выберите сетевой интерфейс для захвата трафика.
3. Начните захват данных.
4. Фильтруйте трафик по типам пакетов, например:

   * ARP: `arp`
   * DNS: `dns`
   * TCP: `tcp`
5. Проанализируйте столбцы «Source» (источник) и «Destination» (назначение) для понимания, кто с кем общается.
6. Используйте дополнительные фильтры для выявления аномалий или проблем.
7. Сохраните результаты для дальнейшего анализа.
